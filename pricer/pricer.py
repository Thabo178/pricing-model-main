...
def _price_note_core(
    note: dict,
    n_paths: int = 50_000,
    seed: int = 42,
    memory: bool = False,
    heston_override: dict = None,
) -> dict:
    ...
    heston_params.setdefault("dividend_yield", 0.0)
    ...
    return {
        ...
        "face_value": note["face_value"],
        "n_paths": n_paths,
        "dividend_yield": round(float(heston_params.get("dividend_yield", 0.0)), 6),
    }
...
def price_worst_of(
    ...
    for ticker in tickers:
        p = dict(load_params(ticker))
        p["risk_free_rate"] = note["risk_free_rate"]
        p.setdefault("dividend_yield", 0.0)
    ...
    return {
        ...
        "n_paths": n_paths,
        "dividend_yields": {
            ticker: round(float(p.get("dividend_yield", 0.0)), 6)
            for ticker, p in zip(tickers, heston_params_list)
        },
    }
