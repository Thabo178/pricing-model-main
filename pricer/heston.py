...
HESTON_PARAMS = {
    'NVDA': {'v0': 0.1225, 'kappa': 2.0, 'theta': 0.0900, 'sigma': 0.50, 'rho': -0.70},
    'TSLA': {'v0': 0.1600, 'kappa': 1.8, 'theta': 0.1200, 'sigma': 0.60, 'rho': -0.65},
    'AMD':  {'v0': 0.1100, 'kappa': 2.0, 'theta': 0.0800, 'sigma': 0.45, 'rho': -0.70},
    'META': {'v0': 0.0900, 'kappa': 2.2, 'theta': 0.0700, 'sigma': 0.40, 'rho': -0.65},
    'GOOGL':{'v0': 0.0700, 'kappa': 2.5, 'theta': 0.0600, 'sigma': 0.35, 'rho': -0.60},
    'AMZN': {'v0': 0.0800, 'kappa': 2.2, 'theta': 0.0650, 'sigma': 0.40, 'rho': -0.65},
    'HOOD': {'v0': 0.2500, 'kappa': 1.5, 'theta': 0.1800, 'sigma': 0.70, 'rho': -0.60},
    'LULU': {'v0': 0.1400, 'kappa': 1.8, 'theta': 0.1000, 'sigma': 0.50, 'rho': -0.65},
    'NOW':  {'v0': 0.0900, 'kappa': 2.0, 'theta': 0.0700, 'sigma': 0.42, 'rho': -0.62},
    'PLTR': {'v0': 0.1800, 'kappa': 1.6, 'theta': 0.1400, 'sigma': 0.65, 'rho': -0.60},
    'WFC':  {'v0': 0.0600, 'kappa': 2.5, 'theta': 0.0500, 'sigma': 0.30, 'rho': -0.55},
    'SPY':  {'v0': 0.0400, 'kappa': 3.0, 'theta': 0.0350, 'sigma': 0.25, 'rho': -0.75},
}

_DEFAULT = {'v0': 0.1000, 'kappa': 2.0, 'theta': 0.0800, 'sigma': 0.45, 'rho': -0.65}
...
def load_params(underlier: str, calibrated_dir: str = None) -> dict:
    ...
    if path.exists():
        ...
        return {k: data[k] for k in ('v0', 'kappa', 'theta', 'sigma', 'rho')}
    return get_heston_params(underlier)
